...
PAGERANK_FILE = "ag/pagerank.tsv"
OUTPUT_FILE = "ag_pagerank.tsv"
...
df_pr = pl.read_csv(PAGERANK_FILE, separator="\t").join(df_map, on='node_id', how='left').join(df_lab, on='address', how='left')

top = df_pr.sort("pagerank", descending=True).head(SIZE).select("node_id", "address", "label", "pagerank").with_row_index("rank")
