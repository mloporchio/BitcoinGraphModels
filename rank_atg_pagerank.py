...
PAGERANK_FILE = "atg/pagerank.tsv"
OUTPUT_FILE = "atg_pagerank.tsv"
...
df_pr = pl.read_csv(PAGERANK_FILE, separator="\t").join(df_map, on='node_id', how='left')

top = df_pr.sort("pagerank", descending=True).head(SIZE).select("node_id", "node_type", "entity", "label", "pagerank").with_row_index("rank")
