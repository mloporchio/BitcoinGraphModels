...
CLUSTERING_FILE = "pg/clustering.tsv"
OUTPUT_FILE = "pg_clustering.tsv"
...
df_pr = pl.read_csv(CLUSTERING_FILE, separator="\t").join(df_node_map, on='node_id', how='left')

top = df_pr.sort("clustering", descending=True).head(SIZE).select("node_id", "tx_hash", "output_id", "clustering").with_row_index("rank")
